...
{
    float t = worldTime;
    if (t >= 21.0f) return PHASE_NIGHT;   // 21-24 wraps to night
    for (int i = PHASE_COUNT - 1; i >= 0; i--)
    {
        if (t >= phaseBoundaries[i]) return i;
    }
    return PHASE_NIGHT;
}
...
float Simulator::getDayPhaseProgress() const
{
    ...
    else end = phaseBoundaries[phase + 1];
...
void Simulator::redraw()
{
    ...
    if (curCI == PHASE_NIGHT) nextCI = 1;  // night→dawn color
...
void Simulator::keyPressed(char k)
{
    ...
    switch (k)
    {
        ...
        case 'n':
        {
            ...
            if (next == PHASE_NIGHT) worldTime = 21.0f;
            dayPhase = next;
            int h = (int)worldTime;
            int m = (int)((worldTime - h) * 60);
            cout << "[" << (h < 10 ? "0" : "") << h << ":" << (m < 10 ? "0" : "") << m << "] "
